# LinkedIn post (single self-contained explainer video attached)

Posting notes: LinkedIn autoplays muted, so the video carries its own text and the
post body does the framing. Lead with the first two lines (they sit above the
"see more" fold). Upload `assets/videos/preflight_explainer.mp4` directly.
...
- The demo interface shots are a product simulation built from real runs; the block
  verdict and receipt shown are genuine and independently verifiable.
